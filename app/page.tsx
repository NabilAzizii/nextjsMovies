...
async function getData() {
  const url = await fetch('https://api.themoviedb.org/3/trending/movie/day', {
    headers: {
      ...
      Authorization: process.env.THEMOVIESDATABASE_API as string,
    },
    next:{
      revalidate: 10,
    }
            ...
            <div className="flex flex-col overflow-hidden rounded-lg border bg-white">
              ...
              <div className="flex flex-1 flex-col p-4 sm:p-6">
                <h2 className="mb-2 text-lg font-semibold text-gray-800">
                  <Link href={`/movie/${movie.id}`} className="transition duration-100 hover:text-teal-500 active:text-teal-600">
                    {movie.title}
                  </Link>
                </h2>
